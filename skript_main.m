...
if (sc2<3)
    Q=[];
    for s=timesteps:-1:1
        Qs=infinit_generator(t(s)); grids=length(Qs);
        ds=sum(Qs,2);       %and entropy part of Q
        ...
        H=diag(exp(-delta_t*ds));  %holding probabilities
        ...
        if (s1==0)
            ...
            if (sc2==0)
                Q=[[(eye(grids)-H)*Qs,H*Q(1:grids,:)];[zeros(s1,grids),Q]];
            end
            ...
            if (sc2==1)
                ...
                Q=[[(eye(grids)-H)*Qs,H*Q(1:grids,:)];[block,1/2*Q(:,1:grids),Q(:,grids+1:end)]];
            end
...
if (sc2==3)
    ...
    weights=rand(1,nr); weights=weights/sum(weights);
    for i=1:nr
        E=eye(timesteps); 
    ...
    Qs=infinit_generator(t(1)); grids=length(Qs);
    Q=zeros(timesteps*grids, timesteps*grids);
    for i=1:timesteps
        Qs=infinit_generator(t(i)); grids=length(Qs);
        ds=sum(Qs,2);       %and entropy part of Q
...
Q=Q-diag(sum(Q,2)); %negative elements are only on the diagonal
Q=sparse(Q);

spectrum=eigs(Q-0.1*speye(size(Q,2)),6,'sm')+0.1

%PCCA+ based on second largest eigenvalue/vector
[u,v]=eigs(Q-0.1*speye(size(Q,2)),2,'sm');
u=u*sign(u(grids,2)); % such that chi is the "right part"
chi=1/(max(u(:,2))-min(u(:,2)))*(u(:,2)-min(u(:,2)));
...
for i=1:timesteps
   plot(density((i-1)*grids+1:i*grids));
   sumdens=sumdens+density((i-1)*grids+1:i*grids);
end
plot(sumdens,'r');
hold off;
